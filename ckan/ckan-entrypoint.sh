...
([ ! -z "$DATASTORE_PASSWORD" ] && export DATASTORE_PASSWORD=$DATASTORE_PASSWORD) || (echo "FATAL: DATASTORE_PASSWORD is not configured" && ERROR=1)
export CKAN_DATASTORE_WRITE_URL=postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_FQDN}/${DATASTORE_DB}
export CKAN_DATASTORE_READ_URL=postgresql://${DATASTORE_ROLENAME}:${DATASTORE_PASSWORD}@${POSTGRES_FQDN}/${DATASTORE_DB}
